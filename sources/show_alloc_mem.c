...
/*   Updated: 2022/11/23 20:21:43 by lucocozz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static size_t	__print_blocks(t_page *page)
{
	...
	for (uint i = 0; i < page->block_count; i++) {
		ft_putstr("  0x");
		ft_putnbr_base((long)((void*)block + sizeof(t_page)), "0123456789ABCDEF");
		ft_putstr(" - 0x");
		ft_putnbr_base((long)((void*)block + block->size), "0123456789ABCDEF");
		ft_putstr(" : ");
		ft_putnbr_base(block->size - sizeof(t_block), "0123456789");
		ft_putstr(" bytes\n");
...
static size_t	__print_pages(t_binding *binder, const char *type)
{
	...
	for (uint i = 0; i < binder->count; i++) {
		ft_putstr(type);
		ft_putstr(" : 0x");
		ft_putnbr_base((long)((void*)page), "0123456789ABCDEF");
		ft_putchar('\n');
...
void	show_alloc_mem(void)
{
	...
	ft_putstr("Total : ");
	ft_putnbr_base(total, "0123456789");
	ft_putstr(" bytes\n");
